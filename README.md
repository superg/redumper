...
## macOS
Before running for the first time, you'll need to install the `llvm` prerequisite via Homebrew: `brew install llvm@18`. Additionally, you may also require removing the quarantine block that macOS may add to the downloaded redumper executable: `xattr -dr com.apple.quarantine {your/path/to/}redumper` (adjust the path to redumper for your machine).
