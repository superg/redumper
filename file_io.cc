...
std::vector<uint8_t> read_vector(const std::filesystem::path &file_path)
{
	...
	if(!fs.is_open())
		throw_line(fmt::format("unable to open file ({})", file_path.filename().string()));
	fs.read((char *)data.data(), data.size());
	if(fs.fail())
		throw_line(fmt::format("read failed ({})", file_path.filename().string()));
...
void write_vector(const std::filesystem::path &file_path, const std::vector<uint8_t> &data)
{
	...
	if(!fs.is_open())
		throw_line(fmt::format("unable to create file ({})", file_path.filename().string()));
	fs.write((char *)data.data(), data.size());
	if(fs.fail())
		throw_line(fmt::format("write failed ({})", file_path.filename().string()));
}
...
uint32_t check_file(const std::filesystem::path &file_path, uint32_t entry_size)
{
	if(!std::filesystem::exists(file_path))
		throw_line(fmt::format("file doesn't exist ({})", file_path.filename().string()));

	if(!std::filesystem::is_regular_file(file_path))
		throw_line(fmt::format("not a regular file ({})", file_path.filename().string()));
