...
std::vector<Area> area_map(SectorReader *sector_reader, uint32_t lba_start, uint32_t lba_end)
{
        ...
        for(uint32_t i = 0; i < zeroed_start;)
        {
            ...
            if(sector_reader->read(directory_extent.data(), pr_offset, dr_sectors_count) == dr_sectors_count)
            {
                ...
                for(auto const &dr : directory_records)
                {
                    ...
                    o = lba_start + dr.second.offset.lsb - sector_reader->sectorsBase();
