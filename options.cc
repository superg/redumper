        ...
        if(o[0] == '-')
        {
            ...
            if(s_value == nullptr && i_value == nullptr)
            {
                ...
                else if(key == "--stop-lba")
                {
                    ...
                    force_qtoc = true;
                else if(key == "--zero-error-sectors")
                    zero_error_sectors = true;
                else if(key == "--skip")
                    s_value = &skip;
...
//    os << "\trings\tscans CD for protection rings, outputs ring ranges for CD dumping" << std::endl;
    ...
    os << "\t--zero-error-sectors\tZero initialize all erroneous sectors, this excludes skipped 0x55 sectors" << std::endl;
